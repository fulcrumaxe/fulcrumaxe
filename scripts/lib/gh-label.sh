...
# Resolve project repo (project.json → LABEL_REPO env → repo-resolve fallback)
# shellcheck source=repo-resolve.sh
source "$(dirname "${BASH_SOURCE[0]}")/repo-resolve.sh"
REPO="${LABEL_REPO:-$(_resolve_repo)}"
...
# _gh_label_owner / _gh_label_name — split REPO ("owner/name") for GraphQL vars.
_gh_label_owner() { echo "${REPO%%/*}"; }
_gh_label_name()  { echo "${REPO##*/}"; }
...
_gh_label_graphql_mutate() {
  ...
  if [[ -z "$pr_id" ]]; then
    echo "${mutation}: could not resolve node id for PR #${pr} in ${REPO}" >&2
    return 1
  fi

  lbl_id="$(_gh_label_label_id "$label")"
  if [[ -z "$lbl_id" ]]; then
    echo "${mutation}: label '${label}' does not exist in ${REPO}" >&2
    return 1
  fi
...
apply_label() {
  ...
  local out rc
  out=$(gh api -X POST "repos/${REPO}/issues/${pr}/labels" -f "labels[]=${label}" 2>&1)
  rc=$?
  if [[ $rc -ne 0 ]]; then
...
remove_label() {
  ...
  out=$(gh api -X DELETE "repos/${REPO}/issues/${pr}/labels/${encoded_label}" 2>&1)
  rc=$?
