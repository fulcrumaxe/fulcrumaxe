...
# Reads .autonomous-team/config.json's "repo" field via the shared resolver
# (or AUTONOMOUS_TEAM_REPO). D#2598 fix-round item 2c: this used to fall back
# to a hard-coded 'autonomous-agent-7/fulcrumaxe' when nothing resolved —
# silently pointing a fork's panel queries at THIS project's own repo instead
# of failing loudly. An unresolved repo is now a loud, empty value: every
# caller of _PANEL_HELPERS_REPO_OWNER/_PANEL_HELPERS_REPO_NAME gets a query
# that visibly fails against an empty owner/name rather than one that quietly
# succeeds against the wrong repo.
# ---------------------------------------------------------------------------
_PANEL_HELPERS_REPO_ROOT="${REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
# shellcheck source=scripts/lib/repo-resolve.sh
source "$_PANEL_HELPERS_REPO_ROOT/scripts/lib/repo-resolve.sh"
_PANEL_HELPERS_REPO="$(_resolve_repo 2>/dev/null || true)"
if [ -z "$_PANEL_HELPERS_REPO" ]; then
  echo "panel-helpers.sh: FATAL: could not resolve a repo slug (checked .autonomous-team/config.json and AUTONOMOUS_TEAM_REPO) — refusing to silently fall back to this project's own repo" >&2
fi
