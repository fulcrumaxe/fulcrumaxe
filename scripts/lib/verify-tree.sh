...
#
# Known affected suites (re-measured 2026-09-19, code-plane/main @ 251c6bcd,
# host nixos — re-verify before trusting a stale entry, and update this list
...
#     assertion that depends on it goes red for the wrong reason. Still
#     unfixed as of this measurement — the differential still reproduces.
#
# Dropped from this list: tests/test_no_hardcoded_checkout_paths_guard.sh. It
# was 32 passed / 10 failed protected vs 42 passed / 0 failed plain at the
# 2026-09-10 measurement; its own fixture helper has since been fixed at the
# source (it now `chmod u+w`s its own mktemp copy instead of relying on an
# inherited writable bit). Re-measured 2026-09-19 at the commit above: 52
# passed / 0 failed in BOTH tree shapes — no differential. The suite has also
# grown since 2026-09-10 (52 cases now vs 42/32 then), which is expected and
# unrelated to this list.
#
# Checked and NOT on this list: tests/test_identifier_rewrite_project_name.sh.
# Re-measured 2026-09-19 at the commit above: 13 passed / 13 failed in BOTH
# tree shapes — no differential (previously reported 11 passed / 15 failed in
# both shapes on 2026-09-10; the suite has changed since, but the "no
# differential" finding still holds). Its failures come from missing expected
# content in files this run's fixtures don't set up, unrelated to write
# protection. Listed here so nobody re-adds it on the strength of an old
# measurement alone.
#
