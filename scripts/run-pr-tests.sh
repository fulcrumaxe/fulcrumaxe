...
BASH_SUITE_DENYLIST=(
  ...
  "tests/test_spawn_agent_file_scope.sh:bug(D#2153) — 8 passed / 2 failed: suite's own git stub matches only when \$1 == worktree, but scripts/lib/worktree-claims.sh now invokes git -C <root> worktree list --porcelain, so \$1 is -C and the stub no longer matches; fixing this means editing the test body, which D#2153's scope restriction on this file forbids"
