...
- **Linux with GNU coreutils** — this is the supported environment, and it's the only one the shell scripts are written against. **macOS and the BSDs are not supported.** They're not blocked either, and most of the team works there: `scripts/lib/platform-compat.sh` carries real GNU/BSD fallbacks for reading file mtimes and sizes, doing date arithmetic, and in-place `sed`, and the scripts that need those go through it. What's left over is the part to know about — a handful of scripts still call `realpath -m`, `date -d`, and `readlink -f` directly, with no BSD equivalent. The most consequential is the containment gate in `scripts/lib/hook-event.sh`: on a `realpath` without `-m` it fails closed, rejecting every event id and stopping the hook rather than resolving anything. That's the safe direction to fail, but it means hook-driven bookkeeping stops working. `coldstart-preflight.sh` runs those three constructs for real and reports `missing prerequisite: GNU coreutils` if the host rejects any of them, so you find out at setup rather than one broken feature at a time weeks later. It probes the tools themselves and never reads `uname` — a macOS host with `brew install coreutils gnu-sed` and the gnubin directories first on `PATH` passes it, and a Linux host with them stripped out fails it.
