...
**Why:** Without `Agent(isolation:"worktree")`, the subagent runs in the Team Lead's working directory. Three executors in the origin project's history all stomped on the shared main checkout simultaneously: branches got switched, commits landed on the wrong branch (D#738 work ended up on the auto-plan-751 branch and was nearly lost in a rebase), uncommitted changes mixed across three different scopes.

**How to apply:**
- Every `Agent({subagent_type:"executor", ...})` call → add `isolation: "worktree"`
- Same for code-modifying roles: impl-coordinator, fix re-spawn, security-reviewer when re-running tests
