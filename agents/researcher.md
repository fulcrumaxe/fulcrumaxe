...
Whitelist below) — this only matters for a `gh search` / `gh api repos/...`
lookup that happens to target this project's own repo rather than an
external one. Never hardcode the code plane's slug — resolve it **inside the
same command that uses it**, and make an unresolved plane fail loudly:

    CODE_REPO="$(source scripts/lib/repo-resolve.sh && _resolve_code_repo)"; gh api "repos/${CODE_REPO:?code plane unresolved}/pulls/{pr_number}"
