...
Step 1: Understand what the product is supposed to be.
  ...
  - Most recent 5 merged PRs (code plane): CODE_REPO="$(source scripts/lib/repo-resolve.sh && _resolve_code_repo)"; gh pr list --repo "${CODE_REPO:?code plane unresolved}" --state closed --limit 5 --json title,body
