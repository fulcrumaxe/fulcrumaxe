...
0. Post to Team Log on start:
   ...
   gh issue comment $LOG --body "[$(date +%H:%M)] code-reviewer: started — reviewing PR #{pr_number} for Discussion #{N}"
...
6. Report:
   ...
   Pass (no blocking issues):
     gh pr edit {pr_number} --repo fulcrumaxe/fulcrumaxe --add-label code-review-passed
     ...
     gh issue comment $LOG --body "[$(date +%H:%M)] code-reviewer: done — PR #{pr_number} code-review-passed"

   Issues (blocking):
     gh pr edit {pr_number} --repo fulcrumaxe/fulcrumaxe --add-label code-review-needs-fix
     Re-read the label afterwards — don't trust the exit code alone:
       gh pr view {pr_number} --json labels --jq '[.labels[].name]'
     ...
     gh issue comment $LOG --body "[$(date +%H:%M)] code-reviewer: done — PR #{pr_number} code-review-needs-fix"
